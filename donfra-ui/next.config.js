...
const nextConfig = {
  ...
  output: 'standalone',
  trailingSlash: false,
  // Transpile ESM-only packages so webpack generates valid chunks for dynamic imports
  transpilePackages: ['yjs', 'y-monaco', 'y-websocket', 'y-protocols', 'lib0', '@excalidraw/excalidraw'],
  async rewrites() {
    return [
